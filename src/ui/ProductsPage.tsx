    ...
    <div className="relative" data-inventory-page>
      ...
      <div className="flex flex-col space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-400 text-sm">Manage your product catalog</p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800/50 p-4 rounded-lg border border-slate-200 dark:border-slate-700">
          <div className="flex flex-col md:flex-row gap-4 items-end">
            <div className="flex-1">
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Search Products</label>
            ...
            <div className="w-full md:w-48">
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Filter by Category</label>
              <select 
        ...
        {error && <div className="bg-red-500/20 border border-red-500/50 text-red-600 dark:text-red-400 text-sm p-3 rounded-md">{error}</div>}

        <div className="overflow-auto border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800/50">
          <table className="min-w-full text-sm">
            <thead className="bg-slate-100 dark:bg-slate-700/50 text-left">
              <tr>
            ...
            <tbody>
              {loading && <tr><td colSpan={6} className="p-8 text-center text-slate-500 dark:text-slate-400">
              ...
              {!loading && filtered.map(p => (
                <tr key={p.id} className="border-t border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/30">
                  <td className="p-3 font-medium text-slate-900 dark:text-white">
                    <div className="flex items-center gap-3">
                      {p.imageUrl ? <img src={p.imageUrl} alt={p.name} className="w-12 h-12 object-cover rounded-md bg-slate-100 dark:bg-slate-700" /> : 
                        <div className="w-12 h-12 rounded-md bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-[10px] text-slate-500 dark:text-slate-400">
                      ...
                      <div>
                        ...
                        {p.description && <div className="text-xs text-slate-500 dark:text-slate-400 max-w-xs truncate">{p.description}</div>}
                      </div>
                    </div>
                  </td>
                  <td className="p-3 font-mono text-primary-600 dark:text-primary-300">{p.currency} {p.price.toFixed(2)}</td>
                  <td className="p-3">
                    <div className={`text-center rounded-full px-2 py-1 text-xs ${
                      p.stock === 0 ? 'bg-red-500/20 text-red-600 dark:text-red-400' : 
                      p.stock < 5 ? 'bg-orange-500/20 text-orange-600 dark:text-orange-400' : 
                  ...
                  <td className="p-3 text-slate-700 dark:text-slate-300">
                    {p.category ? (
                      <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200">
                        {p.category}
                      </span>
                    ) : '-'}
                  </td>
                  <td className="p-3 text-right">
                    <div className="flex gap-2 justify-end">
                      ...
                      <button 
                        onClick={()=>remove(p.id)} 
                        className="text-xs h-8 px-3 rounded-md font-medium border transition-colors border-red-300 text-red-600 hover:bg-red-50 dark:border-red-500/50 dark:text-red-400 dark:hover:bg-red-500/10"
                      >
                        Delete
                      </button>
                    </div>
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-50 flex">
          <div className="fixed inset-0 bg-black/60 dark:bg-black/80" onClick={closeDrawer} />
          <form onSubmit={save} className="relative w-full max-w-md h-full overflow-y-auto bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-700 p-6 flex flex-col gap-5 ml-auto animate-slide-in-right">
            <div className="flex justify-between items-center pb-4 border-b border-slate-200 dark:border-slate-700">
              <h2 className="text-xl font-semibold">{editingId ? 'Edit Product' : 'Add New Product'}</h2>
              <button type="button" onClick={closeDrawer} className="text-slate-400 hover:text-slate-600 dark:hover:text-white">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            {error && <div className="bg-red-500/20 border border-red-500/50 text-red-600 dark:text-red-400 p-3 rounded-md text-sm">{error}</div>}
            ...
            <Field label="Image URL">
              ...
              />
              {draft.imageUrl && (
                <div className="mt-2 p-2 border border-slate-200 dark:border-slate-700 rounded-md bg-slate-50 dark:bg-slate-800 flex items-center gap-3">
                  <img src={draft.imageUrl} alt="Preview" className="w-16 h-16 object-cover rounded-md bg-slate-100 dark:bg-slate-700" />
...
const Field: React.FC<{ label: string; required?: boolean; children: React.ReactNode }> = ({ label, required, children }) => (
  <label className="flex flex-col gap-1 text-sm font-medium text-slate-700 dark:text-slate-300">{label}{required && ' *'}{children}</label>
);
...
const Th: React.FC<{ children: React.ReactNode; onClick?: ()=>void; active?: boolean; dir?: string; className?: string }> = ({ children, onClick, active, dir, className }) => (
  <th onClick={onClick} className={`p-3 text-xs font-semibold select-none ${onClick ? 'cursor-pointer hover:text-primary-500 dark:hover:text-primary-300' : ''} ${active ? 'text-primary-500 dark:text-primary-300' : 'text-slate-600 dark:text-slate-300'} ${className||''}`}>
